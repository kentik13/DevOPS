...
hardware = (ROOT_DIR + "\\data\\hardware")
innodb = (ROOT_DIR + "\\data\\innodb")
mysql = (ROOT_DIR + "\\data\\mysql")
software = (ROOT_DIR + "\\data\\software")
...
class GetCSV:
    ...
    def check_dirs(self):
        ...
        self.chrome = (ROOT_DIR + "\\chromedriver.exe")  # put your path to chrome driver!
        self.download_directory = (ROOT_DIR +"\\Download")  # put your path!

        self.check_dir(self.download_directory)
        self.check_dir(ROOT_DIR + '\\data')
        self.check_dir(ROOT_DIR + '\\data\\hardware')
        self.check_dir(ROOT_DIR + '\\data\\innodb')
        self.check_dir(ROOT_DIR + '\\data\\mysql')
        self.check_dir(ROOT_DIR + '\\data\\software')

        # self.logger.info("DELETE old *.csv files")
        # self.clear_folder.clearallfolders()
        # # subprocess.Popen(["del /s /F" + ROOT_DIR + "\\data\\software\\*.csv"],
        #                  stdout=subprocess.PIPE).communicate()`

    def clrfolder(self,dir):
        filelist = [f for f in os.listdir(dir) if f.endswith(".csv")]
        for f in filelist:
            os.remove(os.path.join(dir, f))


    def clearallfolders(self):
        self.clrfolder(hardware)
        self.clrfolder(innodb)
        self.clrfolder(mysql)
        self.clrfolder(software)
    ...
    def get_csv(self):
        ...
        for property in self.chart:
            ...
            for k, v in property[1].items():
                ...
                time.sleep(2)
                self.driver.find_element(By.XPATH, f"//*[text()='Data options']").click()
